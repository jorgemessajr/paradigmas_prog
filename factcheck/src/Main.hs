...
calcSim t1 t2 = jaccard palavras1 palavras2
  where
    palavras1 = T.words t1
    palavras2 = T.words t2
...
jaccard xs ys = fromIntegral (length inter) / fromIntegral (length uni)
  where
...
topN n q xs= nPrimeiros
  where
    nPrimeiros = take n decrescente
    decrescente = sortBy (flip compare `on` fst) similaridade
    similaridade = map (\x -> (calcSim q x, x)) xs
...
main = do
  ...
  resultados <-  fmap concat (mapM wait as)


  --recupera os top 10 resultados mais similares
